...
В слое шесть полигонов. Сарайшық (objectid 11, КАТО 711110000) слит с Алматы: общая граница 12,9 км, тот же КАТО, сумма площадей 154,72 км² совпадает с опубликованной площадью района Алматы в схеме из пяти районов (154,71 км²). Общие границы упрощены `coverage_simplify` с допуском 25 м, чтобы соседние контуры не разъехались. Максимальное отклонение от исходных полигонов меньше 30 м. Цвет района берётся из `district_scores` ответа `/api/simulate`. Идентификатор Есиля в датасете — `esil`, на карте полигон `yesil`.
