...
Нужен Python 3.11+. В корне репозитория:
...
```bash
.venv/bin/streamlit run frontend/app.py --server.port 8501 --browser.gatherUsageStats false
```
...
1. На вкладке «Старт» укажите название команды и посмотрите базовый балл.
2. На вкладке «Город» изучите паспорта районов и таблицу показателей в баллах и натуральных единицах.
3. На вкладке «План» выберите пять разных мер. Для районной меры укажите район. Бюджет и ошибки проверяет сервер.
4. Отправьте план. «Результат» покажет балл, место среди допустимых планов, разрыв до лучшего, изменения по районам и критические показатели.
5. Запросите проверенное объяснение и советника. «Стресс-тест» показывает последствия семи отдельных событий и стоимость реакции из остатка бюджета. «Рейтинг» сравнивает команды по баллу или устойчивости.

Скриншоты для презентации команды: добавьте кадры вкладок «Город», «План» и «Результат» после финального оформления интерфейса.
