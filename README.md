...
`frontend/src/assets/astana-districts.svg` собран из слоя районов городского ГИС Е-Саулет: [Hosted/raiony](https://gis.esaulet.kz/server/rest/services/Hosted/raiony/FeatureServer/0), выгрузка 23 сентября 2026. Координаты — местная проекция «Astana» в метрах (азимут −0,451110818°, центр 71,42142501666659° в.д., 51,16971010555549° с.ш., масштаб 1,00006382434499, WGS84). В SVG ось Y перевёрнута, север сверху.

В слое шесть полигонов. Сарайшық (objectid 11, КАТО 711110000) слит с Алматы: общая граница 12,9 км, тот же КАТО, сумма площадей 154,72 км² совпадает с опубликованной площадью района Алматы в схеме из пяти районов (154,71 км²). Общие границы упрощены `coverage_simplify` с допуском 25 м, чтобы соседние контуры не разъехались. Максимальное отклонение от исходных полигонов меньше 30 м. Растр в файл не встроен. Цвета на карте в интерфейсе подставляет фронтенд из `d_after` ответа `/simulate`.
